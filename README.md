...
이 저장소의 현재 프로젝트는 부산을 방문하는 일반 여행객을 위한 온라인 예약 조건 AI 주의사항 서비스입니다.

- 사용자는 온라인 예약 페이지에서 Chrome 확장 프로그램을 실행합니다.
- 확장 프로그램은 Chrome Side Panel에서 취소, 환불, 책임, 이용 조건 등 주의사항 카드를 바로 보여줍니다.
- 웹사이트는 같은 분석 결과의 상세 보기, 샘플 실행, 기록 관리 화면으로 사용합니다.
- 사진 촬영, 종이 서약서 OCR, 해양 레저 전용 흐름은 초기 기획이며 현재 구현 우선순위가 아닙니다.
- 구현 기준은 `AGENT.md`, 실행 순서는 `process.md`를 따릅니다.
